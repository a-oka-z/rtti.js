...
function module_to_source(current_module) {
  let s='';
  function output( l ) {
    s = s+l;
  }
  ...
  if ( current_module.header ) {
    output( current_module.header );
  }
  output( '\n' );
  output( 'schema.define\`\n' );
  output( current_module.validator_list.map( (e,i)=>`${e?.[SCHEMA_VALIDATOR_SOURCE]},\n` ).join('\n') );
  output( '`' );

  if ( current_module.footer ) {
    output( current_module.footer );
  }
...
function BEGIN_MODULE({ module_name =null, header=null, footer=null, filename, dirname }) {
