...
- v2.0.1 fixed the issue that `statement compiler` throws an error when a comma
         is placed after a specified statement.

- v2.0.2 now it is available to access to the `script` property to get its
         souce code for the validators which are generated by factories from
         statement-compiler script.

         (Fri, 06 Jan 2023 22:06:22 +0900)
